...
def TextBox(myDict, myKey, txt, pwd=False):
    if pwd:
        try:
            myDict[myKey] = st.text_input(
                label=txt, type="password", value=myDict[myKey])
        except KeyError:
...
def SelectBox(myDict, myKey, opts, txt, lamKey=None):
    ...
    if lamKey is None:
        ...
        try:
            myDict[myKey] = st.selectbox(
                txt, sortedOpts, format_func=lambda x: x[lamKey], index=sortedOpts.index(myDict[myKey]))
        except KeyError:
            myDict[myKey] = st.selectbox(
                txt, sortedOpts, format_func=lambda x: x[lamKey])
        except ValueError:
            myDict[myKey] = st.selectbox(
                txt, sortedOpts, format_func=lambda x: x[lamKey])
...
def Slider(myDict, myKey, myMin, myMax, txt):
    try:
        myDict[myKey] = st.slider(
            txt, min_value=myMin, max_value=myMax, value=myDict[myKey])
    except KeyError:  # default min value
        myDict[myKey] = st.slider(
            txt, min_value=myMin, max_value=myMax, value=myMin)
